...
def curl(url: str, source_format: SrcFormat = SrcFormat.HTML, destination_format: DstFormat = DstFormat.AS_IS) -> Union[str, dict]:
    ...
    if source_format == SrcFormat.JSON:
        if destination_format == DstFormat.AS_IS:
            ...
            raise ValueError("Unsupported destination format for JSON source. Use 'as-is'.")
    ...
    if destination_format == DstFormat.AS_IS:
        ...
        raise ValueError("Unsupported destination format. Use 'as-is', 'plain', 'markdown', or 'json'.")
